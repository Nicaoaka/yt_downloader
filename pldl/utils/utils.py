...
def sort_by_other[T, K](
        ...
        other: Sequence[K],
        /, *,
        key: Callable[[K], SupportsRichComparison],
        reverse: bool = False
) -> list[T]: ...
@overload
def sort_by_other[T, K: SupportsRichComparison](
        main: Sequence[T],
        other: Sequence[K],
        /, *,
        key: None = None,
...
def sort_by_other[T, K](
        ...
        other: Sequence[K],
        /, *,
        key: Callable[[K], SupportsRichComparison]|None = None,
        reverse: bool = False
) -> list[T]:
    if len(main) != len(other):
        raise ValueError(f"main[{len(main)}] and other[{len(other)}] must have same len")
    pair_key = (lambda pair: key(pair[0])) if key is not None else (lambda pair: pair[0])
    return [x for _, x in sorted(zip(other, main), key=pair_key, reverse=reverse)]
